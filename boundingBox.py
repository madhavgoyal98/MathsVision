...
def isEquationMark(boundingBox, boundingBox1):
    (x, y), (xw, yh) = boundingBox
    (x1, y1), (xw1, yh1) = boundingBox1
    # 20 is a migical number
    return isHorizontalBar(boundingBox) and isHorizontalBar(boundingBox1) and abs(x1 - x) < 20 and abs(xw1 - xw) < 20
...
def isDots(boundingBox, boundingBox1, boundingBox2):
    ...
    cenY1 = y1 + (yh1 - y1) / 2
    cenY2 = y2 + (yh2 - y2) / 2
    # 30 is a migical number
    return (isDot(boundingBox) and isDot(boundingBox1) and isDot(boundingBox2) and max(cenY, cenY1, cenY2) - min(cenY, cenY1, cenY2) < 50)
...
def isPM(boundingBox, boundingBox1):
    ...
    cenX = x + (xw - x) / 2
    cenX1 = x1 + (xw1 - x1) / 2
    case1 = isHorizontalBar(boundingBox) and isSquare(
        boundingBox1) and x < cenX1 < xw and -15 < y - yh1 < 35 and xw - cenX1 < 50
    case2 = isSquare(boundingBox) and isHorizontalBar(
        boundingBox1) and x1 < cenX < xw1 and -15 < y1 - yh < 35 and xw1 - cenX < 50
...
def isFraction(boundingBox, boundingBox1, boundingBox2):
    ...
    cenX1 = x1 + (xw1 - x1) / 2
    cenX2 = x2 + (xw2 - x2) / 2
    case1 = not isDot(boundingBox) and not isDot(boundingBox1) and isHorizontalBar(
        boundingBox2) and (y < y2 < yh1 or y1 < y2 < yh)
    case2 = not isDot(boundingBox2) and not isDot(boundingBox) and isHorizontalBar(
        boundingBox1) and (y2 < y1 < yh or y < y1 < yh2)
    case3 = not isDot(boundingBox1) and not isDot(boundingBox2) and isHorizontalBar(
...
#    im[im < 127] = 0
    ...
    ret, thresh = cv2.threshold(
        imgrey, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # RETR_EXTERNAL for only bounding outer box
...
#             print([(x2, y2), (xw2, yh2)])
        ...
        if (equation or letterI or pm) and not fraction:
            finalRes.append([(min(x, x1), min(y, y1)),
                             (max(xw, xw1), max(yh, yh1))])
            i += 2
        elif (divisionMark or dots) and not fraction:
            finalRes.append([(min(x, x1, x2), min(y, y1, y2)),
                             (max(xw, xw1, xw2), max(yh, yh1, yh2))])
            i += 3
        else:
...
def saveImages(im, boxes, im_name):
    ...
    num = 1
    boxes = sorted(boxes, key=lambda box: (
    ...
    for box in boxes:
        ...
        if w < 25 and h < 25 and float(w)/h < 1.5 and float(h)/w < 1.5:
            symbolImage.save(equal_boxed_path + im_name + "_" + "dot" + "_" +
                             str(y) + "_" + str(yh) + "_" + str(x) + "_" + str(xw) + ".png")
        else:
            # save rectangled element
            symbolImage.save(equal_boxed_path + im_name + "_" + str(num) + "_" +
                             str(y) + "_" + str(yh) + "_" + str(x) + "_" + str(xw) + ".png")
        # fill the found part with black to reduce effect to other crop
        draw = ImageDraw.Draw(image)
        draw.rectangle((x, y, xw, yh), fill='white')
...
def main():
    ...
    for im_name in image_list:
        ...
        rawRes = initialBoxes(im)  # raw bounding boxes
        # connect i, division mark, equation mark, ellipsis
...
#im_name = "C:\\Users\\madha\\Desktop\\Handwritten-Equation-Recognition-Tensorflow-master\\data\\handwriting-math.png"
# im = cv2.imread(im_name)  # specify the image to process
# rawRes =  initialBoxes(im)  # raw bounding boxes
